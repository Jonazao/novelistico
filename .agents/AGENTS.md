...
5. **Bitácora Incremental por Escena:** Inmediatamente después de finalizar y aprobar la redacción de una escena `escena_[M]_final.md` (se considera aprobada cuando el escritor da su visto bueno explícito en el chat o introduce el comando para redactar la escena siguiente), el agente debe crear o actualizar de forma autónoma el archivo de bitácora del capítulo (`capitulos/capitulo_[N]/bitacora_capitulo.md`). Esta bitácora resumirá de manera estructurada los hechos, estados de personajes y entornos introducidos. Al redactar las escenas posteriores, el agente leerá obligatoriamente esta bitácora acumulativa y la escaleta para garantizar la consistencia, evitando releer la prosa extensa de escenas anteriores.
